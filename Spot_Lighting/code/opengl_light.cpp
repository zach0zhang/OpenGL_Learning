...
bool Light::GetUniformLocations()
{
	...
	m_numSpotLightsLocation = glGetUniformLocation(m_shaderProg, "gNumSpotLights");
...
void Light::SetSpotLights(unsigned int NumLights, const SpotLight* pLights)
{
	...
	for (unsigned int i = 0; i < NumLights; i++) {
		...
		Vector3f Direction;
		CopyVector3(Direction,pLights[i].Direction);
		NormalizeVector3(Direction);
		glUniform3f(m_spotLightsLocation[i].Direction, Direction[0], Direction[1], Direction[2]);
		glUniform1f(m_spotLightsLocation[i].Cutoff, cosf(DegToRad(pLights[i].Cutoff)));
